...
**PepoMote 1.9:** buttons that respond the way you expect. **Keep pressed when leaving the button** (on by default on Android: 2 no longer lets go by itself in Mario Kart) and **Slide to press**, two new settings on Android, iPhone/iPad and mobile Linux; a **collapsible header** on the sideways controllers, the Wii U / Switch one included, that keeps the buttons clear and the Keyboard always at hand; the sensor selector switches instantly and **Connect** shows the PC you are already linked to right away. Includes the 1.8.5 features: several Wii Remotes at once in Wii U mode, pointing by tilting without a gyroscope, Home and **Closer** in Dolphin, and the 1.8 ones: an Android server for Dolphin and Eden, guided setup and pairing by code or across networks. [Downloads and release notes](https://github.com/pepitolas13/PepoMote/releases/tag/v1.9.0).
