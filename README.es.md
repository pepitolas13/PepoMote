...
- **macOS** (1.6, beta): el mismo receptor en Mac con chip Apple — cursor y teclas por Accesibilidad, doble pantalla de Cemu, icono en la barra de menús, arranque con el sistema, `.app` firmado en un DMG
- **Aviso de versión nueva** (1.6): todas las apps avisan cuando sale una versión, con el enlace a la release. La comprobación es una consulta a GitHub una vez al día (solo la página de la última versión; no se envía nada tuyo) y se apaga en Ajustes. PepoMote no habla con nada más fuera de tu red.
- **iPad y tablets** (1.6): el mando, el pad NES, el Nunchuk y el GamePad de Wii U crecen hasta llenar la pantalla (misma regla en iOS y Android; los móviles quedan exactamente como estaban). Y en Ajustes está **GamePad sin pantalla táctil**: quita la pantalla del GamePad de Wii U (y la doble pantalla) para que sticks, cruceta y A/B/X/Y sean mucho más grandes; en un móvil pasan a ir uno al lado del otro, como en un Pro Controller
