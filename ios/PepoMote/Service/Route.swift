...
enum Route {
    ...
    /// como con el botón Home. La misma regla que en Android.
    static func showsMedia(_ link: UiLink, _ everywhere: Bool) -> Bool {
        everywhere || (link.connected.map { $0.mode == LinkState.modePointer } ?? true)
    }
