...
**RetroArch (desde 1.10, cambio aditivo):** `ok.modes` anuncia `retroarch`. En ese modo el receptor no usa el DSU: traduce cada `INPUT` al **mando en red** de RetroArch (UDP `127.0.0.1:55400 + slot`, un datagrama `remote_message` de 20 bytes por control, dosificado a uno por jugador y fotograma con el reloj de la propia interfaz de comandos de RetroArch; `desktop/src/retroarch/`) y las teclas rápidas al **puerto de comandos** (`127.0.0.1:55355`). Tres mandos por móvil (`pad`): `retropad` (el apaisado de dos sticks, mismo `INPUT` de 80 bytes que en Switch: A/B/X/Y → A/B/X/Y del RetroPad, L/R, ZL/ZR → L2/R2, clics de stick → L3/R3, − / + → Select/Start, cruceta y los dos sticks; Home = menú de RetroArch, bit 28 = avance rápido mientras se mantiene), `nes` (el Mando Wii vertical u horizontal con puntero del SO para el slot 0, 72 bytes: 1 → B, 2 → A, A → X, B → Y, − / +, cruceta tal como se ve en pantalla —sin el giro de Dolphin—, Home = menú) y `gun` (el Mando de Wii apuntando: el puntero mueve el ratón del SO como en modo puntero, B = clic izquierdo (gatillo), A = clic derecho (recarga), y 1, 2, − / +, cruceta y Home como en `nes`; solo apunta el slot 0). Cada móvil con rol de mando es un jugador (slot + 1); el rol Nunchuk no tiene papel. Los bits 17, 18, 27, 29 (salvo en `nes` y `gun`) y 30 se ignoran. El receptor escribe `retroarch.cfg` con RetroArch cerrado (`docs/SETUP-RETROARCH.md`). El receptor nunca pregunta el estado a RetroArch 1.22.2 ni anteriores (`GET_STATUS` los cierra); qué juego corre lo lee de su historial y lo anuncia con `game` (arriba), y el móvil elige con ello una plantilla de consola: todas las plantillas mandan el mismo `INPUT` de 80 bytes del `retropad`, y cada botón en pantalla emite el bit cuyo id del RetroPad espera el mapeo por defecto del núcleo (p. ej. Mega Drive A/B/C → bits Y/B/A); la tabla completa está en `protocol/retro-layouts.json`, generada desde `pmp::retro`. El **servidor Android** de esta integración también anuncia `retroarch`: mismo mando en red (`127.0.0.1:55400 + slot`) y mismos comandos (55355) contra el RetroArch del propio móvil (`android/app/src/main/java/dev/pepotech/pepomote/server/retro/`); `hello.pad` y `pad` solo aceptan `retropad` y `nes` (`gun` devuelve el mando vigente y un `notice`: no hay ratón); `game` sale del historial de la carpeta `RetroArch` elegida por SAF (sin ficha `.info`: consola por núcleo, extensión y `db_name`); y el modo es automático: cuando RetroArch empieza a responder, `mode retroarch` con `by:"pc"` a todas las sesiones, y cuando un emulador nuevo pide mandos por DSU con RetroArch mudo, vuelve al último modo con `by:"pc"`.

**Mando universal (cambio aditivo):** `ok.modes` anuncia `gamepad`. Lo ofrece solo el receptor de PC (Windows y Linux); el servidor Android no, porque no puede crear un mando virtual. En ese modo el receptor no usa el DSU, no inyecta nada en el SO y **no configura ningún programa**: escribe el `INPUT` directamente en el **mando de Xbox 360 virtual** que ya crea para la vibración (ViGEmBus en Windows, uinput en Linux), y cualquier juego lo ve como un mando de verdad sin tocar su configuración. Mapa: A/B/X/Y → A/B/X/Y de XInput **por posición en pantalla** (la plantilla del móvil pone A abajo, B a la derecha, X a la izquierda e Y arriba, así que lo que se lee es lo que recibe el juego), cruceta → cruceta, L/R → LB/RB, ZL/ZR → los gatillos analógicos a fondo, − / + → Back/Start, clics de stick → L3/R3, Home → Guide, y los dos sticks. `INPUT` de 80 bytes, como en Switch. El **giroscopio mueve el stick derecho** (apuntado por giro), salvo que el dedo esté moviendo el stick derecho de la pantalla, que manda sobre el giro; con `FLAG_TILT` (móvil sin giroscopio de verdad) el giro no apunta. Los bits 9, 10, 17, 18, 27, 28, 29, 30 y los multimedia se ignoran. Un jugador que deja de mandar `INPUT` durante 1 s vuelve a reposo, para que un móvil que se va al fondo no le deje un botón clavado al juego, y al irse el móvil su mando virtual se desenchufa. Sin el driver (`ok.rumble` distinto de `"ready"`) el modo se anuncia igual pero no hay mando: la ventana del receptor enseña cómo instalarlo.
...
| 4 | 1 | u8 | tipo = `0x01` |
| 5 | 1 | u8 | flags: bit0 = quaternion válido (el móvil tiene GAME_ROTATION_VECTOR); bit1 = stick válido (Nunchuk o stick izquierdo del GamePad); bit2 = `FLAG_EXT`, el paquete mide 80 bytes y trae el bloque extendido; bit3 = `FLAG_TOUCH`, hay un dedo en la pantalla táctil del GamePad; bit4 = `FLAG_TILT`, apuntado por inclinación (el receptor saca el cursor del acelerómetro; solo con `ok.tilt`); bits5–6 = rotación del marco 0/90/180/270 (solo con `ok.frame_rotation`); bit7 reservado 0 |
...
**Marco de sensores (bits5–6; cambio aditivo):** el receptor de escritorio anuncia `"frame_rotation":true` en `ok`. Solo entonces el emisor puede incluir la rotación EFECTIVAMENTE aplicada a quat/gyro/accel (0, 1, 2, 3, desplazada 5 bits). El Nunchuk sin remapeo conserva cero. El valor se repite en cada paquete, de modo que perder un datagrama no pierde el cambio. Al variar, el receptor resiembra proyección, filtros y sesgo y recentra; conserva el monitor seleccionado. El teclado congela pose y marco juntos. Sin la capacidad se envían los bits a cero, también hacia servidores Android anteriores que rechazan flags desconocidos.
...
### 4.5 `RUMBLE` receptor→móvil — 20 bytes (desde 1.12, aditivo)

La vibración que pide el juego. Va por el **mismo socket UDP** que el `INPUT`, a la dirección que el receptor aprendió de los `INPUT` de esa sesión, así que no hace falta abrir nada.

Layout (20 bytes): bytes 0-7 como en `INPUT` (magic `PMP1`, `tipo` = `0x04`, `flags` = 0, reservado), bytes 8-11 `session_id`, 12-15 u32 LE `seq`, byte 16 `strong` (motor grande, 0-255), byte 17 `weak` (motor pequeño, 0-255), bytes 18-19 u16 LE `ttl_ms`.

Es **estado, no evento**: mientras el juego vibra, el receptor repite el paquete cada 100 ms con el `seq` al alza, y al parar manda unos cuantos con `strong` = `weak` = 0 y `ttl_ms` = 0 (la cola de ceros, por si se pierde alguno). El móvil:

- descarta un `seq` viejo o repetido (resta como entero con signo de 32 bits ≤ 0), lo que también cubre el desbordamiento;
- toma el nivel como `max(strong, weak) / 255`, y le aplica la escala del ajuste «Vibración en los juegos» (`high` 1, `normal` 0,65, `low` 0,35, `off` 0) **en cada orden al motor**, para que cambiar el ajuste valga al instante;
...
Como el móvil vibra exactamente mientras el receptor crea el motor encendido, el receptor no puede perder ningún «apaga» ni quedarse con uno que nadie va a mandar. En Windows deja varias peticiones de aviso esperando en el driver ViGEmBus: Dolphin escribe cada cambio en dos llamadas seguidas (motor L y luego R), y con una sola petición un ViGEmBus anterior a 1.17.333 tiraba el segundo aviso (el móvil vibraba sin fin desde la primera vibración; los nuevos lo perdían de tarde en tarde). Y como en XInput lo último que se escribe en el mando se queda para siempre (Dolphin no apaga el Mando de Wii al pausar ni al cerrar el juego), en Windows un motor encendido sin ningún aviso del emulador durante 10 s se da por acabado: sale el cero y su cola. Es la cota que Dolphin pone a sus propias vibraciones en mandos SDL y evdev; en Linux no hace falta, porque los efectos de uinput traen su duración y el núcleo los borra al cerrarse el programa.
