...
- **Un pad sin muestras frescas hay que seguir contestándolo** (PadData con la cabecera en «desconectado» y el cuerpo a cero, cada 500 ms). Cemu NO re-pide por reloj: pide sus pads una vez al arrancar y, a partir de ahí, su única forma de volver a pedir uno es que le contestemos ese pad (medido: 1.218.620 peticiones en 43 s para un pad que contesta, y exactamente 2 —las del arranque— para uno que no, ninguna más en 45 s ni aunque empezara a contestar después). Sin ese latido, un móvil callado más de los 3 s de caducidad —pantalla apagada, app al fondo, un bache de wifi, o sencillamente conectarse con Cemu ya abierto— perdía la suscripción y Cemu no volvía a pedir ese pad NUNCA: ese mando quedaba muerto hasta reiniciar Cemu (bug real de 1.10.0, reportado con un GamePad y dos Mandos Wii en Nintendo Land).
