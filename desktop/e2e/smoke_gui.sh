...
# el receptor se relance (exec de sí mismo con render por software). Vale con
...
# xvfb.log.
#
#   uso: smoke_gui.sh wayland|x11 [fallback]
set -euo pipefail
MODE="${1:-wayland}"
FALLBACK="${2:-}"
...
case "$MODE" in wayland | x11) ;; *) echo "uso: $0 wayland|x11 [fallback]"; exit 2 ;; esac
rm -rf "$OUT"
...
ENV=(PEPOMOTE_CONFIG_DIR="$OUT/config" PEPOMOTE_PORT=$PORT PEPOMOTE_DSU_PORT=26780
     ...
     PEPOMOTE_SMOKE=2500 PEPOMOTE_DEBUG=1)
...
echo "-- receptor.log:"; grep -E "Ventana|PEPOMOTE_SMOKE|PÁNICO|\[eframe|\[winit|\[glutin|\[egui_glow" "$LOG" 2>/dev/null || true
